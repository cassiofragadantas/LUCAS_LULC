# TabTransformer repo: https://github.com/lucidrains/tab-transformer-pytorch
import torch
...
config_details = "TabTransf_GeoInfo_" + suffix + loo + '_Lev' + str(pred_level) + '_' + str(epochs) + 'ep_seed' + str(rng_seed)
...
train_data, train_label, test_data, test_label, \
    climate_train, climate_test, _, _, train_lat_long, test_lat_long = loadData(data_path, suffix, pred_level, loo_region)

# Integrate lat-long info to features
train_data = np.concatenate([train_data, train_lat_long], axis=1)
test_data = np.concatenate([test_data, test_lat_long], axis=1)
...
# Filter out NaN values from climate_train and climate_test
valid_indices = ~pd.isna(climate_train)  # Use pandas to handle NaN
print(f'Removing {pd.isna(climate_train).sum()} samples with NaN climate regions in training data')
climate_train = climate_train[valid_indices]
train_data = train_data[valid_indices]
train_label = train_label[valid_indices]

valid_indices = ~pd.isna(climate_test)  # Use pandas to handle NaN
print(f'Removing {pd.isna(climate_test).sum()} samples with NaN climate regions in training data')
climate_test = climate_test[valid_indices]
test_data = test_data[valid_indices]
test_label = test_label[valid_indices]

# Map climate regions to integer codes
climate_train = pd.Series(climate_train)
climate_test = pd.Series(climate_test) # convert to Pandas if not already the case
cat_type = pd.api.types.CategoricalDtype(categories=climate_train.unique())
climate_train = climate_train.astype(cat_type).cat.codes
climate_test  = climate_test.astype(cat_type).cat.codes
# Treat case of unseen categories (regions) at training. Assing unseen to last index
has_unseen = (climate_test == -1).any()
climate_test  = climate_test.mask(climate_test < 0, climate_train.nunique())

n_classes = len(np.unique(train_label))
n_climate = len(np.unique(climate_train)) + (1 if has_unseen else 0)

print(f'train_data shape: {train_data.shape}')
print(f'train_label shape: {train_label.shape}')
print(f'climate_train shape: {climate_train.shape}')
print(f'test_data shape: {test_data.shape}')
print(f'test_label shape: {test_label.shape}')
print(f'climate_test shape: {climate_test.shape}')

x_train = torch.tensor(train_data, dtype=torch.float32)
y_train = torch.tensor(train_label, dtype=torch.int64)
categ_train = torch.tensor(climate_train.values, dtype=torch.long).unsqueeze(1)
...
categ_test = torch.tensor(climate_test.values, dtype=torch.long).unsqueeze(1)

train_dataset = TensorDataset(x_train, y_train, categ_train)
# valid_dataset = TensorDataset(x_valid, y_valid)
test_dataset = TensorDataset(x_test, y_test, categ_test)
...
model = TabTransformer(
    categories = (n_climate,),          # tuple containing the number of unique values within each category
...
if os.path.isfile(model_name):
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch, x_categ in train_dataloader:
...
print(f'TabTransf_GeoInfo TEST perf: Acc={acc*100:.2f}, F1={f1*100:.2f}')
