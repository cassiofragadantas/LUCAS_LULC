...
import numpy as np
import pandas as pd
from misc import MLPDisentanglePos, SupervisedContrastiveLoss, normalizeFeatures, loadData, cumulate_EMA, evaluation
...
model_name = "model_MLP_posEnc_" + suffix + '_Lev' + str(pred_level) + '_' + str(epochs) + 'ep_seed' + str(rng_seed) + '.pth'
...
train_data, train_label, test_data, test_label, \
    climate_train, train_geo_enc, test_geo_enc = loadData(data_path, suffix, pred_level)
...
train_target_geo_enc = train_geo_enc[idx_med]

train_source_data = train_data[~idx_med]
train_source_label = train_label[~idx_med]
train_source_geo_enc = train_geo_enc[~idx_med]
...
train_geo_enc = np.concatenate([train_target_geo_enc, train_source_geo_enc],axis=0)
...
# 2) Ours
model = MLPDisentanglePos(n_classes).to(device) #act_out=False
