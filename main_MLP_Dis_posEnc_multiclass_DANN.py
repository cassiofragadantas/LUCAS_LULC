...
import numpy as np
import pandas as pd
from misc import MLPDisentanglePosDANN, SupervisedContrastiveLoss, normalizeFeatures, loadData, cumulate_EMA, evaluation
...
config_details = "MLP_DisMultiDANN_posEnc_" + suffix + loo + '_Lev' + str(pred_level) + '_' + str(epochs) + 'ep_seed' + str(rng_seed)
...
if os.path.isfile(model_name):
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch, dom_batch, coord_batch in train_dataloader:
            ...
            optimizer.zero_grad()
            pred, inv_emb, spec_emb_d, spec_d_pred, inv_emb_n1, spec_emb_n1, inv_fc_feat, spec_fc_feat, discr_pred = model(x_batch, coord_batch)

            ##### DOMAIN CLASSIFICATION #####
            loss_ce_spec_dom = loss_fn(spec_d_pred, dom_batch)

            ##### DANN DOMAIN CLASSIFICATION #####
            loss_ce_dann = loss_fn(discr_pred, dom_batch)

            ##### ORTHOGONALITY LOSS #####
            emb_inv = F.normalize(inv_fc_feat)
            emb_spec = F.normalize(spec_fc_feat)
            ...
            loss = loss_fn(pred, y_batch) + contra_loss + loss_ce_spec_dom + loss_ce_dann

            loss.backward() # backward pass: backpropagate the prediction loss
            optimizer.step() # gradient descent: adjust the parameters by the gradients collected in the backward pass
            tot_loss+= loss.cpu().detach().numpy()
            contra_tot_loss+= contra_loss.cpu().detach().numpy()
            dann_tot_loss += loss_ce_dann.cpu().detach().numpy()
            den+=1.
